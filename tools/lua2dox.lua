...
local function checkComment4Var(line)
	local pos1, pos2 = string.find(line, "%-%-%-")
	if pos1 and pos2 and string.sub(line, pos2+1, pos2+1) ~= '-' then
		-- 只匹配 "---"
		local var_str = string_trim(string.sub(line, 1, pos1-1))
		local tail_str = string_trim(string.sub(line, pos2+1))
		local code_str
		if string.find(var_str,'^local%s+') then
			-- local 转换成static
			code_str = string.gsub(var_str,"^local", "static")
		else
			code_str = string.gsub(var_str, "%.", " ")
			--code_str = string.match(var_str, "[^%.%s]+%s*=%s*%S+")
		end
		code_str = string_trim(code_str)
		if string.sub(code_str, -1) == '{' then
			code_str = code_str .. '}'
		end
		return code_str .."; //!< " .. tail_str
	end
end
...
-- 			TCore_Debug_show_var('line',line )
			if string.sub(line,1,2)=='--' then -- its a comment
				local char3 = string.sub(line,3,3)
				local char4 = string.sub(line,4,4)
				-- 兼容 "---" 和 "--!"
				if char3=='!' or (char3=='-' and char4~='-') then -- it's a magic comment
					local magic = string.sub(line,4)
					local macro, tail = getMagicDirective(magic)
					if macro == "package" then
						-- 将一个lua模块(table)转换成class
						outStream:writeln("class " .. tail .."{};")
					else
			...
			elseif string.find(line,'=%s*[Cc]lass%(') then
				...
				outStream:writeln('class ' .. klass .. parent_str .. '{};')
			else
				-- we don't know what this line means, so we can probably just comment it out
				if #line>0 then
					local conv_str = checkComment4Var(line)
					if conv_str then
